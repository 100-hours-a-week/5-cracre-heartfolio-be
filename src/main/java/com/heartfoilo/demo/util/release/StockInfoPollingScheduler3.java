...
public class StockInfoPollingScheduler3 {
    ...
    @Value("${korea-invest.app-key3}")
    private String appKey;

    @Value("${korea-invest.app-secret3}")
    ...
    private void updateStockInfo(String type, String symbol) {
        try {
            ...
            System.out.println("price3 :"+redisUtil.getStockInfoTemplate(symbol).getCurPrice());
    ...
    private void getStocks(){
        stocks = stockRepository.findAll().stream()
            .filter(a -> a.getId() > 24 && a.getId() <= 36).toList();
    }
}
