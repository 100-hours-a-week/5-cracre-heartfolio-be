...
public class StockInfoPollingScheduler4 {
    ...
    @Value("${korea-invest.app-key4}")
    private String appKey;

    @Value("${korea-invest.app-secret4}")
    ...
    private void updateStockInfo(String type, String symbol) {
        try {
            ...
            System.out.println("price4 :"+redisUtil.getStockInfoTemplate(symbol).getCurPrice());
    ...
    private void getStocks(){
        stocks = stockRepository.findAll().stream()
            .filter(a -> a.getId() > 36 && a.getId() <= 49).toList();
    }
}
